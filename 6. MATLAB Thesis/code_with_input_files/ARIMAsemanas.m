...
Q=Q(:);
Q=reshape(Q,7,365);Qsem=nanmean(Q,1)'; %promedios semanales
Q18(366:371,1)=NaN;Q18=reshape(Q18,7,53);Qsem18=nanmean(Q18,1)';

D = LagOp({1 -1},'Lags',[0,52]);
dY = filter(D,Qsem);
Y=dY;
...
% parcorr(Y,'NumLags',110,'NumSTD',2) 
% figure(2)
% autocorr(Y,'NumLags',110,'NumSTD',2)
...
for Q=1:2
      for P=1:2
         for p=1:3
 Mdl(p,P,Q) = arima('ARLags',p,'SARLags',52:52:P*52,'SMALags',52:52:Q*52,'Seasonality',52);
 pp(p,P,Q)=p;
 PP(p,P,Q)=P;
 QQ(p,P,Q)=Q;
         end
...
[~,bic] = aicbic(LOGL,BIC1+1,365);
[M,I] = min(bic); % el mejor es el SARIMA(1,0,0)(2,1,2)52 ----posicion 10

EstMdl = estimate(Mdl(10),y);
[yF,yMSE] = forecast(EstMdl,53,'Y0',y); %Pronostico MMSE
[YMc,E] = simulate(EstMdl,53,'NumPaths',1000,'Y0',y); %Pronostico montecarlo
Qperc=[quantile(YMc,0.025,2) quantile(YMc,0.25,2) quantile(YMc,0.50,2) quantile(YMc,0.75,2) quantile(YMc,0.975,2)];

por1=zeros(53,1);
for i=1:53
    if Qsem18(i,1) <= Qperc(i,4) && Qsem18(i,1) >= Qperc(i,2) 
     por1(i,1)=1;
    end
end
por2=zeros(53,1);
for i=1:53
    if Qsem18(i,1) <= Qperc(i,5) && Qsem18(i,1) >= Qperc(i,1)
     por2(i,1)=1;
    end
end

x=[1:53];Q18=Qsem18;Qp=YMc;
...
t4=title('Distribución de las predicciones para cada semana');
set(t4,'FontSize',16);
xlabel('Tiempo [semanas]');ylabel('Caudal [m3/s]'); set(gca,'FontSize',14);
l2=legend([h1 w3 w10 h2 w4 w11 w5], 'Rango entre percentiles 2.5-97.5',...
    ['Porcentaje de puntos en el rango= ' num2str(sum(por2)*100/53,'%.2f') ' %'],...
    ['Tamaño medio del rango= ' num2str(mean(Qperc(:,5)-Qperc(:,1)),'%.2f') ' m3/s'],...
    'Rango intercuartil',...
        ['Porcentaje de puntos en el rango= ' num2str(sum(por1)*100/53,'%.2f') ' %'],...
...
% xlabel('Tiempo [semanas]'); ylabel('Caudal [m3/s]'); set(gca,'FontSize',12)
...
% xlabel('Tiempo [semanas]');ylabel('%');
% set(gca,'FontSize',12)
% 
% figure(5)
% subplot(2,2,[1,3]);
% histfit(Q18-mean(Qp,2),20)
