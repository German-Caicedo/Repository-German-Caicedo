...
r(1,1)=corr(Q(365,:)',Q(1,:)','rows','complete');

b=zeros(365,1);
for i=2:365
b(i,1)=r(i,1)*s(i,1)/s(i-1,1);
end
b(1,1)=r(1,1)*s(1,1)/s(365,1);

k=Q(365,7);
Qp=zeros(365,1000);
for j=1:1000
% Componente aleatorio
t = 0 + sqrt(1)*randn(365,1);
%Caudales 2018
Qp(1,j)=Qm(1,1)+b(1,1)*(k-Qm(365,1))+t(1,1)*s(1,1)*(1-r(1,1)^2)^0.5;
for i=2:365
...
% set(l1,'FontSize',15);
% t1=title('Serie cada día'); set(t1,'FontSize' ,15);
% xlabel('Tiempo [días]'); ylabel('Caudal [m3/s]'); set(gca,'FontSize',14)
% hold off
% 
% figure(2)
% subplot(2,1,1);
% histogram(Qpr,13:2:340)
% t2=title('Distribución de caudales');set(t2,'FontSize',16);
% ylabel('Caudal [m3/s]'); set(gca,'FontSize',14)
% subplot(2,1,2); 
% histogram(log(Qpr),2.5:.05:5.9)
...
t4=title('Distribución de las predicciones para cada día');
set(t4,'FontSize',16);
xlabel('Tiempo [días]');ylabel('Caudal [m3/s]'); set(gca,'FontSize',14);
l2=legend([h1 w3 w10 h2 w4 w11 w5], 'Rango entre percentiles 2.5-97.5',...
    ['Porcentaje de puntos en el rango= ' num2str(sum(por2)*100/365,'%.2f') ' %'],...
    ['Tamaño medio del rango= ' num2str(mean(Qperc(:,5)-Qperc(:,1)),'%.2f') ' m3/s'],...
    'Rango intercuartil',...
        ['Porcentaje de puntos en el rango= ' num2str(sum(por1)*100/365,'%.2f') ' %'],...
...
% xlabel('Tiempo [días]'); ylabel('Caudal [m3/s]'); set(gca,'FontSize',12)
...
% xlabel('Tiempo [días]');ylabel('%');
% set(gca,'FontSize',12)
% 
% figure(5)
% subplot(2,2,[1,3]);
% histfit(Q18-mean(Qp,2),100)
...
% 
% X=zeros(31,12,2);
% X(1:dm(1,1),1,1)=Q18(1:dm(1,1),1);
% for i=1:11
% X(1:(dm(i+1,1)-dm(i,1)),i+1,1)=Q18(dm(i,1)+1:dm(i+1,1),1);
% end
% X(1:dm(1,1),1,2)=Qperc(1:dm(1,1),3);
% for i=1:11
% X(1:(dm(i+1,1)-dm(i,1)),i+1,2)=Qperc(dm(i,1)+1:dm(i+1,1),3);
% end
% 
